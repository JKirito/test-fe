...
import './HomeProgress.scss'; // Import the new SCSS file

export interface IHomeProgress {
  currentStepNumber: number; // Renamed prop
  totalSteps: number; // Renamed prop
...
export function HomeProgress({
  totalSteps,
  currentStepNumber, // Use renamed prop
      ...
      if (targetCompletedCount > prevCompletedCount) {
        ...
        stepsToFade.forEach((stepNumber) => {
          const fadingStep = document.querySelector(
            `.home-progress-list__item:nth-child(${stepNumber})`
          );
          fadingStep?.classList.add('home-progress-list__item--fade-out');
        });
      ...
      <ul className="home-progress-list">
        ...
        {Array.from({ length: totalSteps }, (_, i) => i + 1).map((stepNumber) => (
          <li
            ...
            className={`home-progress-list__item e-br-32 ${
              completedStepsVisual.includes(stepNumber) ? 'home-progress-list__item--active' : ''
            }`}
          ></li>
        ))}
      </ul>
